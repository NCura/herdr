...
herdr terminal session observe-pane <pane_id> [--cols N] [--rows N]
...
`terminal session observe-pane` is the strict integration path. It accepts only
the current canonical pane ID returned by Herdr, with no terminal ID, agent
label, alias, stale-ID, or current-pane fallback. The observer is bound to that
pane, terminal ID, and terminal runtime generation and closes if any identity
changes. With neither `--cols` nor `--rows`, it uses the authoritative pane
runtime dimensions as view-only rendering geometry. If either flag is supplied,
it keeps fixed geometry; the omitted axis remains at its existing 120-column or
40-row default. Neither mode resizes the PTY. Dimensions are limited to 320
columns, 120 rows, and 20,000 total cells and are rejected rather than clamped.
The first `terminal.frame` is full with sequence 1, later
frames are contiguous and coalesced when the writer is blocked. Binary wire
frames and encoded NDJSON records are each limited to 2 MB; ANSI payloads use
the lower advertised `max_ansi_bytes` limit so base64 and JSON framing fit.
`terminal.closed`
includes one of `invalid_request`, `pane_unavailable`, `pane_changed`,
`frame_too_large`, `protocol_error`, or `detached`. Disconnecting an observer
does not send input, resize or scroll the PTY, change controller ownership, or
stop the PTY process. Check `capabilities.terminal_observation` in `ping` or
`herdr status server --json` before using this contract.
