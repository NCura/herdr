...
changes. Its dimensions affect only the rendered view and never resize the PTY.
Dimensions are limited to 320 columns, 120 rows, and 20,000 total cells; the
default is 120x40. The first `terminal.frame` is full with sequence 1, later
