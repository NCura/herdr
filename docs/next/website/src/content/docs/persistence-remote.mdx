...
```bash
herdr terminal session observe-pane w1:p1 --cols 120 --rows 40
```

This path rejects generic targets and identity aliases, keeps its dimensions
view-only, sends a full sequence-1 frame first, bounds and coalesces output, and
